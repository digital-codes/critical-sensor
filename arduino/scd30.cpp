/*
  ...
  https://github.com/sparkfun/SCD30
...
bool SCD30::begin(uint16_t intervall)
{
   ...
   *
   * The default clock stretch (maximum wait time) on the ESP8266-library (2.4.2) is 230us which is set during Wire.begin();
  ...
  {
    if (intervall > 0) {
      setMeasurementInterval(intervall);    //seconds between measurements
...
bool SCD30::readMeasurement()
{
  ...
  Wire.write(COMMAND_READ_MEASUREMENT >> 8);   //MSB
  Wire.write(COMMAND_READ_MEASUREMENT & 0xFF); //LSB
  if (Wire.endTransmission() != 0)
    return (0); //Sensor did not ACK

  const uint8_t receivedBytes = Wire.requestFrom((uint8_t)SCD30_ADDRESS, (uint8_t)18);
  bool error = false;
  if (Wire.available())
  {
    byte bytesToCrc[2];
...
uint16_t SCD30::readRegister(uint16_t registerAddress)
{
  ...
  Wire.requestFrom((uint8_t)SCD30_ADDRESS, (uint8_t)2);
  if (Wire.available())
  {
